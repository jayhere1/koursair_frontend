...
import {AlertTriangle,ChevronUp,ChevronDown,X,Plus,Heart,Info,} from "lucide-react";
...
import { PassportUploader } from "./PassportUploader"; // Adjust path as needed
...
const KenyaBookingPage = () => {
  // Authentication Protection moved here
  ...
  const totalAmount = adultCount * ADULT_PRICE_DYNAMIC + childCount * CHILD_PRICE_DYNAMIC;
  ...
  // --- Validation helpers (Copied from original) ---
  ...
  const validateRequiredFields = () => {
    ...
    for (let i = 1; i < travelers.length; i++) {
      const t = travelers[i];
      const idx = i + 1;
      if (!t.firstName || !isValidName(t.firstName)) return { valid: false, firstInvalid: `traveler_${idx}_firstName` };
      if (!t.lastName || !isValidName(t.lastName)) return { valid: false, firstInvalid: `traveler_${idx}_lastName` };
      ...
      if (!t.nationality || !isValidName(t.nationality)) return { valid: false, firstInvalid: `traveler_${idx}_nationality` };
    }
    ...
    if (!isValidName(bookingData.emergencyName)) return { valid: false, firstInvalid: "emergencyName" };
    if (!isValidPhone(bookingData.emergencyContact || "")) return { valid: false, firstInvalid: "emergencyPhone" };
    if (!bookingData.medicalRestrictions.trim()) return { valid: false, firstInvalid: "medicalRestrictions" };
    if (!bookingData.mealPreference) return { valid: false, firstInvalid: "mealPreference" };
    if (bookingData.mealPreference === "Other" && !bookingData.mealPreferenceOther.trim()) return { valid: false, firstInvalid: "mealPreferenceOther" };
    if (!bookingData.mobilityIssues || bookingData.mobilityIssues.length === 0) return { valid: false, firstInvalid: "mobilityIssues" };
    if (bookingData.mobilityIssues.includes("Other") && !bookingData.mobilityOther.trim()) return { valid: false, firstInvalid: "mobilityOther" };
    if (!bookingData.allergies || bookingData.allergies.length === 0) return { valid: false, firstInvalid: "allergies" };
    ...
    for (let i = 0; i < travelers.length; i++) {
      ...
      if (isFutureDate(t.dob)) {
        return { valid: false, firstInvalid: i === 0 ? "primaryDobInvalid" : `traveler_${i + 1}_dobInvalid` };
      }
  ...
  const focusField = (key?: string | number) => {
    ...
    if (k === "contactName") scrollToEl(contactNameRef.current);
    if (k === "emergencyName") scrollToEl(emergencyNameRef.current);
    // ... add other specific scroll logic here if needed (simplified for brevity)
  ...
  const payToday = (() => {
    ...
    if (paymentPlan === "HALF") return Math.round((totalAmount - halfDiscount) / 2);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    let newValue = value;
    if (name === "emergencyName") newValue = value.replace(/[0-9]/g, "");
    if (name === "mobile") newValue = value.replace(/\D/g, "").slice(0, 15);
    setBookingData((prev) => ({ ...prev, [name]: newValue }));
  };

  const handleTravelerChange = (id: number, field: keyof PassportGuest, value: string) => {
    if (firstInvalidField) setFirstInvalidField(null);
    const sanitizedFields = ["firstName", "middleName", "lastName", "nationality"];
    const newValue = sanitizedFields.includes(field as string) ? value.replace(/[0-9]/g, "") : value;
    setTravelers((prev) => prev.map((t) => (t.id === id ? { ...t, [field]: newValue } : t)));
  };
  ...
  const handlePassportUpload = async (id: number, file: File) => {
    ...
    try {
      ...
      setTravelers((prev) => prev.map((t) => t.id === id ? { ...t, passport: url, passportFileName: file.name, passportFile: null } : t));
    } catch  {
      setLocalPopup({ message: "Passport upload failed.", type: "error" });
    } finally {
  ...
  const handlePassportDelete = (id: number) => {
    setTravelers((prev) => prev.map((t) => t.id === id ? { ...t, passportFile: null, passportFileName: undefined, passport: undefined } : t));
  };
 ...
 const handleRegistrationSubmit = async (
  ...
  const handleAllergyChange = (allergy: string) => {
    setBookingData((prev) => {
      const allergies = prev.allergies.includes(allergy) ? prev.allergies.filter((a) => a !== allergy) : [...prev.allergies, allergy];
  ...
  const handleMobilityChange = (issue: string) => {
    setBookingData((prev) => {
      if (issue === "None") return { ...prev, mobilityIssues: ["None"], mobilityOther: "" };
      const issues = prev.mobilityIssues.includes(issue) ? prev.mobilityIssues.filter((i) => i !== issue) : [...prev.mobilityIssues.filter((i) => i !== "None"), issue];
                ...
                <div className="bg-[#fffafa] rounded-3xl shadow-2xl border p-6">
                  ...
                  <div className="flex items-start gap-2 mb-4 p-3 rounded-lg border bg-blue-50 border-blue-200">
                    ...
                    <p className="text-xs font-medium text-blue-800 leading-snug">
                      {rateType === "EARLY"
                        ? "Early Bird pricing is applied. You are saving more by booking before Mar 31."
...
export default KenyaBookingPage;